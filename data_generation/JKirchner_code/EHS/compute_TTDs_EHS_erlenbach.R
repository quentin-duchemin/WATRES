...
source("./code/EHS_code.R")   #replace filename as needed for future versions
...
for (idxquantile in 0:3){
  ...
  input_filename <- paste('./weekly_data/', paste(STATION, paste(MODE,'_weekly_data.txt', sep=""),  sep=""), sep="");
  
  ############################
  # here we read the data file 
  
  data <- read.delim(input_filename, header=TRUE, sep=" ", na.strings=c("NA",".","","#N/A"))    #this is the test data file
  ...
  p <- data$J      #precipitation water flux
  ...
  nttd <- 10   # number of time steps for TTDs
  ...
  output_label <- paste('./weekly_data/resultsTTDs/', paste(STATION, paste(MODE,'_weekly_data', sep=""),  sep=""), sep="");
  
  
  Cp <- data$CJ    #precipitation tracer concentration (or del value)
  Cq <- data$Cout    #streamflow tracer concentration (or del value)
  ...
  # write file of TTDs
  write.table(data.frame(TTD_lag, QquantileTTD$TTD, QquantileTTD$TTD_se), paste(output_label, paste("quantile", paste( as.character(idxquantile), ".txt", sep=""), sep=""), sep="_"), row.names=FALSE, sep=" ")
