  ...
  for (i=nstart; i<n; i++) {
    for (k=0; k<nttd; k++) {
      ...
      if (k==0) y0 = 0.0;
        ...
        if (i<=nstartsave) y0 = TTD_Su(0,k-1); else  y0 = TTD_Su(i-1-nstartsave, k-1) ; 
      }
      ...
      // LOWER
      b = R[i];
      if (i<=nstartsave) a = R[i] * TTD_D(0,k); else  a = R[i] * TTD_D(i-1-nstartsave,k);
      if (k==0) y0 = 0.0;
      else {
        if (i<=nstartsave) y0 = TTD_Sl(0,k-1); else  y0 = TTD_Sl(i-1-nstartsave, k-1) ; 
      }
    ...
    // CHANNEL
      ...
      if (k==0) y0 = 0.0;
        ...
        if (i<=nstartsave) y0 = TTD(0,k-1); else  y0 = TTD(i-1-nstartsave, k-1) ; 
      }
...
{  
  ...
  //and calculate ttd's
  model_age_tracking(nstart, nstartsave, n, su_ref, sl_ref, sc_ref, nttd, p, et, qOF, qSS, qGW, q, qOFa, qSSa, qGWa, qa, R, su, sl, sc, TTD_Su, TTD_Sl, TTD_SSa, TTD_GWa, TTD, fwd_TTD, TTDa, fwd_TTDa);
