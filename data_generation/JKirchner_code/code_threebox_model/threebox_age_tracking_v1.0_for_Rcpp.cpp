...
void model_age_tracking(int n, 
                        ...
                        NumericMatrix &TTDa, NumericMatrix &fwd_TTDa
)
  ...
  
{
  ...
  initial_TTD_su[0] = (1.0 - exp(-dt*(p_bar-qOF_bar)/su_ref));
  initial_TTD_D[0] = 1.0 - (su_ref/((p_bar-qOF_bar)*dt))*(1.0 - exp(-(p_bar-qOF_bar)*dt/su_ref));  //we will need this one for the lower box
  for (k=1; k<nttd; k++) {
    initial_TTD_su[k] = initial_TTD_su[k-1] * exp(-(p_bar-qOF_bar)*dt/su_ref); 
    initial_TTD_D[k] = initial_TTD_su[k-1] * (su_ref/((p_bar-qOF_bar)*dt))*(1.0 - exp(-(p_bar-qOF_bar)*dt/su_ref));
  }
  ...
  for (i=0; i<n; i++) {
    for (k=0; k<nttd; k++) {
      ...
      if (k==0) y0 = 0.0;
        ...
        if (i==0) y0 = initial_TTD_su[k-1]; else  y0 = TTD_Su(i-1, k-1) ; 
      }
      if (i==0) h = su_ref; else h = su[i-1];
      c = p[i] - qOFa[i] - qSSa[i] - R[i];
      
      TTD_Su(i,k) = integral( a, b, y0, c, h, dt );
      TTD_D(i,k) = avg_integral( a, b, y0, c, h, dt );
    } 
  }
  ...
  /////////////////////    LOWER BOX    /////////////////////////////////////////////////////////////////////////////
  //calculate ttd for lower box (fraction of lower box storage that is composed of same-time-step precip (lag0) and precip from 1, 2, 3... time steps ago
  ...
  //first calculate the intial ttd, assuming a steady-state equilibrium with upper box at constant input rate of R_bar and storage of sl_ref
  
  initial_TTD_sl[0] = initial_TTD_D[0]  * (1.0 - exp(-qGW_bar*dt/sl_ref));
  initial_TTD_GW[0] = initial_TTD_D[0] * (1.0 - sl_ref/(qGW_bar * dt) * (1.0 - exp(-qGW_bar*dt/sl_ref)));
    for (k=1; k<nttd; k++) {
      initial_TTD_sl[k] = initial_TTD_D[k] + (initial_TTD_sl[k-1] - initial_TTD_D[k]) * exp(-qGW_bar*dt/sl_ref);
      initial_TTD_GW[k] = initial_TTD_D[k] + (initial_TTD_sl[k-1] - initial_TTD_D[k]) * (sl_ref/(qGW_bar * dt)) * (1.0 - exp(-qGW_bar*dt/sl_ref));   // we need this for initializing the channel box
    }
    ...
    for (i=0; i<n; i++) {
      for (k=0; k<nttd; k++) {
        ...
        if (k==0) y0 = 0.0;
          ...
          if (i==0) y0 = initial_TTD_sl[k-1]; else  y0 = TTD_Sl(i-1, k-1) ; 
        }
        if (i==0) h = sl_ref; else h = sl[i-1];
        c = R[i] - qGWa[i];
        
        TTD_Sl(i,k) = integral( a, b, y0, c, h, dt );
        TTD_GWa(i,k) = avg_integral( a, b, y0, c, h, dt );
      } 
    }
  ...
  /////////////////////    CHANNEL BOX    /////////////////////////////////////////////////////////////////////////////
  //calculate ttd for channel box (fraction of channel storage that is composed of same-time-step precip (lag0) and precip from 1, 2, 3... time steps ago
  ...
  initial_TTD_sc[0] = initial_TTD_I[0]  * (1.0 - exp(-q_bar*dt/sc_ref));
  for (k=1; k<nttd; k++) {
    initial_TTD_I[k] = (qSS_bar*initial_TTD_D[k] + qGW_bar*initial_TTD_GW[k])/(qOF_bar + qSS_bar + qGW_bar);
    initial_TTD_sc[k] = initial_TTD_I[k] + (initial_TTD_sc[k-1] - initial_TTD_I[k]) * exp(-q_bar*dt/sc_ref);
  }
  ...
  for (i=0; i<n; i++) {
    for (k=0; k<nttd; k++) {
      ...
      if (k==0) a = qOFa[i]+qSSa[i]*TTD_D(i,k)+qGWa[i]*TTD_GWa(i,k);
      else a = qSSa[i]*TTD_D(i,k)+qGWa[i]*TTD_GWa(i,k);
      if (k==0) y0 = 0.0;
      else {
        if (i==0) y0 = initial_TTD_sc[k-1]; else  y0 = TTD(i-1, k-1) ; 
      }
      if (i==0) h = sc_ref; else h = sc[i-1];
      c = qOFa[i]+qSSa[i]+qGWa[i]-qa[i];
      
      TTD(i,k) = integral( a, b, y0, c, h, dt );
      TTDa(i,k) = avg_integral( a, b, y0, c, h, dt );
    } //next k
  } //next i
  ...
  for (i=0; i<n; i++) {
    for (k=0; k<nttd; k++) {
      if ((i+k)<n) {if (p[i]>0.0) fwd_TTD(i,k) = TTD(i+k,k)*q[i+k]/p[i]; else fwd_TTD(i,k) = NA_REAL;}
      if ((i+k)<n) {if (p[i]>0.0) fwd_TTDa(i,k) = TTDa(i+k,k)*qa[i+k]/p[i]; else fwd_TTDa(i,k) = NA_REAL;}
    } //next k
  } //next i
  
  
  //for precipitation times too close to end to allow complete forward ttd, fill with NA
  for (i=n-nttd; i<n; i++) for (k=0; k<nttd; k++) { fwd_TTD(i,k) = NA_REAL; fwd_TTDa(i,k) = NA_REAL; }
  
  //for discharge times too close to beginning for complete ttd, fill with NA
  for (i=0; i<nttd; i++) for (k=0; k<nttd; k++) { TTD(i,k) = NA_REAL; TTDa(i,k) = NA_REAL; } 
...
{  
  ...
  NumericMatrix TTD(n,nttd);      //transit time distribution (instantaneous at end of time step)
  NumericMatrix fwd_TTD(n,nttd);	//forward transit time distribution (instantaneous at end of time step)
  NumericMatrix TTDa(n,nttd);      //transit time distribution (averaged over time step)
  NumericMatrix fwd_TTDa(n,nttd);	//forward transit time distribution (averaged over time step)
  NumericMatrix TTD_GWa(n,nttd);   //transit time distribution of drainage from lower box (averaged over time step)
  NumericMatrix TTD_SSa(n,nttd);	  //transit time distribution of drainage from upper box (averaged over time step)
  NumericMatrix TTD_Su(n,nttd);   //transit time distribution of upper box (instantaneous at end of time step)
  NumericMatrix TTD_Sl(n,nttd);	  //transit time distribution of lower box (instantaneous at end of time step)
  ...
  //and calculate ttd's
  model_age_tracking(n, su_ref, sl_ref, sc_ref, nttd, p, et, qOF, qSS, qGW, q, qOFa, qSSa, qGWa, qa, R, su, sl, sc, TTD_Su, TTD_Sl, TTD_SSa, TTD_GWa, TTD, fwd_TTD, TTDa, fwd_TTDa);
